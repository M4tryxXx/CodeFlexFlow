...
import { Message, UserFullType, ChatDataType } from "app/lib/types";
...
const ChatSocket = ({ data }: { data: ChatDataType }) => {
  const {
    messagesDb,
    senderId,
    limiter,
  ...
  useEffect(() => {
    ...
    const updateMessages = async () => {
      let refreshedMessages: any = await getConversation(user.id);
      let updatedMessages: Message[] = [];
      refreshedMessages.forEach((message: Message) => {
        if (
