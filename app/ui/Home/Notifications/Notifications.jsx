...
import { mark_message_read, delete_message_read } from "../../../lib/actions";
...
export default function Notifications({ user }) {
  ...
  const getNotificationsList = (unreadList) => {
    let userList = [];
    let usersObj = [];

    const notificationsList = unreadList.map((notification) => {
      if (
        userList.find((n) => {
  ...
  const readNotifications = readNotificationsList?.map((notification) => {
    if (notification.read) {
      return (
        <Tooltip
          key={notification.id}
              ...
              <>
                {getNotificationsList(notifications).length > 0 ? (
                  <>
                    ...
                    {getNotificationsList(notifications).map((notification) => {
                      return (
                        <Tooltip
