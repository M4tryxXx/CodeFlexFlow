...
export default function Messages({ messages_data, conversations }: any) {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (activeConversation[0].from_user_id !== user.id) {
      const new_message = {
        subject: "Re: " + activeConversation[0].subject,
        message: message,
        from_user_id: user.id,
        to_user_id: activeConversation[0].from_user_id,
        from: user.username,
        to: activeConversation[0].from,
      };
      console.log("new_message 1: ", activeConversation);
      ...
      setMessage("");
      setLoading(false);
      let currentConversationWith = activeConversation[0].from;
      if (activeConversation[0].from_user_id === user.id) {
        currentConversationWith = activeConversation[0].to;
      } else {
        currentConversationWith = activeConversation[0].from;
      }
      ...
      const new_message = {
        subject: activeConversation[0].subject,
        message: message,
        from_user_id: user.id,
        to_user_id: activeConversation[0].to_user_id,
        from: user.username,
        to: activeConversation[0].to,
      };
      console.log("new_message 2: ", activeConversation);
        ...
        <div
          ...
          onClick={async () => {
            if (!loading) {
              ...
              setActiveConversation(conversationsState[conversation]);
            ...
            onClick={async (e) => {
              ...
              if (!loading) {
                ...
                const response = await delete_message_read(
                  conversations[conversation][0].id
                );
                if (response) {
